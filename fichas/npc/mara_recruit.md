# Mara — NPC Secundário (recruta)

**Tipo:** Recruta do Pack (med/hidroponia)  
**Facção / contexto:** Pack Badlands (integração em andamento)  
**Status:** Ativo — integrada

> Apresentada por [Lina Park](lina_park.md). Sobrenome não confirmado na campanha.

---

## Personalidade

- Olhar atento de quem já passou por muita coisa e ainda está de pé.
- Direta ao falar; Ryan a considerou a mais confiável dos três candidatos.
- Prática em ambientes hostis; foco em medicina de campo e cultivo.

## Aparência / voz (rápido)

- *(Detalhes físicos não fixados — manter tom competente e calmo.)*

## Eventos narrativos

| Data (aprox.) | Evento |
| ------------- | ------ |
| 30/06–01/07/2026 | Selecionada por Lina; aceita proposta de pack móvel com proteção. |
| 01/07/2026 | Apresentada a Reyes; integração social com Valk no almoço. |
| 02/07/2026 | Participa da montagem de tendas; avaliação positiva na reunião estratégica. |
| 03/07/2026 | Observada por Ryan de manhã carregando suprimentos com o pack, mantendo certa distância social (ainda não notou Ryan). |

## Relação com a crew

- **Ryan:** Avaliação positiva; confiança inicial alta.
- **Valk:** Ajudou na integração social no acampamento.

## Notas para o narrador

- Papel previsto: medicina de campo + hidroponia (estufa modular futura).

---

## Referências

- [Lina Park](lina_park.md) · [Pack Badlands](../../facoes/pack_badlands.md)
- [Sessão 003](../../logs/sessao_resumo_003.md) · [Sessão 005](../../logs/sessao_resumo_005.md)
- [Board](../../board/board_campanha.md)
