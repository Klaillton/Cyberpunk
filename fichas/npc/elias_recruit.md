# Elias — NPC Secundário (recruta)

**Tipo:** Recruta do Pack (refinaria / processamento)  
**Facção / contexto:** Pack Badlands (integração em andamento)  
**Status:** Ativo — integrado; **recomendado para destilaria**

> **Não confundir** com **Dr. Elias Vossler** (Job 001 — ver [dr_elias_vossler.md](dr_elias_vossler.md)).  
> Apresentado por [Lina Park](lina_park.md). Sobrenome não confirmado.

---

## Personalidade

- Prático, um pouco cansado; lado operacional gosta de mobilidade e proteção do pack.
- Relaxa quando entende o papel (manutenção, processamento químico).
- Menos volátil que Tomas; avaliação inicial positiva.

## Aparência / voz (rápido)

- *(Detalhes físicos não fixados — tom de técnico de refinaria experiente.)*

## Eventos narrativos

| Data (aprox.) | Evento |
| ------------- | ------ |
| 30/06–01/07/2026 | Ex-técnico de refinaria; recrutado via Lina para o pack. |
| 01/07/2026 | Apresentado a Reyes com foco em processamento e manutenção pesada. |
| 02/07/2026 | Ryan recomenda para **destilaria** na reunião com Reyes e Tio Gringo. |
| 03/07/2026 | Observado por Ryan de manhã na área da destilaria improvisada, trabalhando com ferramentas, concentrado e útil (ainda não notou Ryan). |

## Relação com a crew

- **Ryan:** Recomendação explícita para destilaria.
- **Valk:** Integração logística na viagem de volta.

## Notas para o narrador

- Gancho narrativo: refinaria móvel / destilaria do pack.

---

## Referências

- [Lina Park](lina_park.md) · [Pack Badlands](../../facoes/pack_badlands.md)
- [Sessão 003](../../logs/sessao_resumo_003.md) · [Sessão 005](../../logs/sessao_resumo_005.md)
- [Board](../../board/board_campanha.md)
