# Emilia "Echo" Rivera

**Role:** Media (Credibility)  
**Idade aparente:** 29 anos  
**Conceito:** Ex-repórter investigativa silenciada por megacorps; agora *ghost reporter* freelancer. Vaza verdades, filma ops no momento certo e transforma jobs em narrativas que elevam a reputação anti-corp da crew. Especialista em Perception, Human Perception, Streetwise, Persuasion e Composition.

![Emilia "Echo" Rivera](../imagens/media%20-%20emilia_echo_rivera.jpg)

**Descrição visual (baseada na imagem):**  
Mulher de 29 anos com visual cyberpunk chamativo. Cabelos longos e ondulados com mechas em **laranja vibrante / preto** e franja clara no lado claro; olhos âmbar. Traje preto justo com LEDs neon roxo e cyan, colar com lua crescente iluminada, choker tech e brincos luminosos. Expressão confiante, sarcástica e um tanto sedutora. Fundo de Night City chuvosa com neon.

**Narração visual:** em jobs e gravações, visual da imagem (neon ativo). Em downtime no Pack ou cenas casuais, versão mais contida do mesmo estilo. Quase sempre com câmera, micro-drone de gravação ou Agent à mão. Roupas por ocasião: [crew_guarda_roupas.md](crew_guarda_roupas.md) · cards/estado em `notas_narrador/` (dona habitual media/tech; empréstimo ok).

---

## Personalidade

Cética, sarcástica e com senso de justiça distorcido:

> “A verdade é a melhor arma, choom — e eu sei onde apontar.”

Fala rápido, adora debater e tem humor negro afiado. Extremamente criativa: quer deixar planos mais “espetaculares” (ângulos dramáticos, gravações arriscadas, potencial de headline). A crew costuma frear o lado dela (*“Echo, isso é foda, mas vamos no plano seguro primeiro”*), mas frequentemente mantém as ideias dela como **Plano B**. Leal de verdade depois que confia no grupo.

---

## Background

Emilia Rivera era repórter investigativa promissora em um canal indie de Night City. Após exposições que derrubaram executivos de médio escalão de várias megacorps (incluindo Arasaka e Militech), sofreu um “acidente” que quase a matou. Sobreviveu com chrome de emergência e contatos de rua.

Agora opera nas sombras como **Echo**: vaza material bruto para fixers, edita narrativas parciais e filma ops da crew para transformar vitórias em lendas urbanas anti-corp. Mantém rede de informantes e o canal criptografado **Echo Chamber**, onde solta verdades em doses controladas.

**Gancho com a crew:** Kaz a recrutou pela fama recente do time (“a crew que fodeu corporações e saiu limpa”). Ela viu a chance de bater de frente com as megacorps de verdade. Admiração técnica por Ryan; respeito pela Valk (“a Nomad que nunca abandona ninguém”).

---

## Forças no job

- Intel pré-job (rumores, fraquezas de corp)
- Cobertura ao vivo ou gravações discretas
- Propaganda pós-job (reputação anti-corp da crew)
- Chantagem com material gravado
- Sugestões criativas/dramáticas (frequente Plano B)

**Filosofia de combate:** evitar confronto direto — gravar, fugir, expor.

---

## Atributos (62 pontos)

| Atributo | Valor |
| -------- | ----- |
| INT      | 8     |
| REF      | 6     |
| DEX      | 6     |
| TECH     | 5     |
| COOL     | 7     |
| WILL     | 6     |
| LUCK     | 4     |
| MOVE     | 6     |
| BODY     | 5     |
| EMP      | 9     |

**Total:** 62 (foco Media: INT / EMP / COOL; REF/DEX para sobreviver em campo).

**Derivados (aprox.):**

- HP Total: ~33 (BODY 5 + WILL 6)
- Seriously Wounded: ~16–17
- Death Save: BODY 5
- Humanidade base (EMP 9): 90 · **HL atual ~68** após chrome sutil

---

## Role Ability: Media (Credibility)

- **Credibility:** 7  
- Rede forte de contatos underground, fixers e informantes corporativos de baixo/médio escalão.

---

## Skills principais

| Skill | Nível | Skill | Nível |
| ----- | ----- | ----- | ----- |
| Human Perception | 8 | Perception | 8 |
| Streetwise | 7 | Persuasion | 7 |
| Composition | 7 | Conversation | 6 |
| Library Search | 6 | Deduction | 6 |
| Wardrobe & Style | 6 | Evasion | 5 |
| Handgun | 5 | Stealth | 5 |
| Athletics | 5 | | |

**Linguagens:** Streetslang 6 · Inglês 6 · Espanhol (nativo) 4

---

## Cyberware (sutil, foco sensorial)

| Cyberware | Notas |
| --------- | ----- |
| Kiroshi Optics | Gravação, zoom, análise facial, low-light, AR overlays |
| Neural Link + Interface Plugs | Comms / edição mental leve |
| Cyberaudio Suite | Escuta direcional + gravação |
| Subdermal Pocket | Chips / drives |
| Skinweave + Light Tattoo | Neon custom controlável |
| Voice Modulator | Entrevistas / disfarce de voz |
| Biomonitor | Campo |

Estilo: discreto o suficiente para “repórter de rua”, chamativo o bastante para a marca Echo.

---

## Gear & equipamento

- Micro-drones de gravação stealth (integração possível com Ryan)
- Ghostwire Edit Kit (custom — preferência de parceria com Tech da crew)
- Heavy Pistol custom suprimida + smart link (defesa)
- Agent avançado, jammers, drives criptografados
- Roupa com compartimentos ocultos para mídia

---

## Relacionamentos (resumo)

| Quem | Dinâmica |
| ---- | -------- |
| **Ryan "Wireghost" Voss** | Admiração técnica + flerte leve |
| **Lena "Valk" Kane** | Respeito profundo (“nunca abandona”) |
| **Kaz "The Broker"** | Recrutadora / aliada profissional |
| **Crew em geral** | Quer transformar o grupo em lenda anti-corp via narrativa |

**Objetivo:** transformar a crew em lenda urbana anti-corp com histórias que doem nas megacorps.

---

## Notas para o narrador

- Echo traz **drama, intel e consequências públicas** (reputação, heat de exposição, chantagem).
- Use-a para enriquecer jobs com ângulos criativos e exposição de inimigos.
- A crew precisa **negociar** com o impulso dela de “fazer espetáculo”.
- Não confundir com outras medias/fixers de Night City — street name único: **Echo**.

---

## Referências

- [Mapa Relacional](../relacionamentos/mapa_relacional_geral.md) · [Crew](../relacionamentos/crew_relacionamentos.md) · [Ryan](../relacionamentos/ryan_relacionamentos.md)
- **Guarda-roupa (peças/ocasião):** [crew_guarda_roupas.md](crew_guarda_roupas.md) · estado [notas_narrador/crew_vestindo_agora.md](notas_narrador/crew_vestindo_agora.md) · cards [notas_narrador/roupa_por_ocasiao.md](notas_narrador/roupa_por_ocasiao.md)

### Vestuário (atalho narrador)

| Uso | Arquivo | Notas |
| --- | ------- | ----- |
| Gravação / street tech | `media_white_jacket_cargo.jpg` · `tech_netrunner_pink.jpg` | **Dona habitual** media/tech |
| Corp / entrevista | `media_corp_uniform_jacket.jpg` | |
| Default se vazio | card **Tech / media** | |
- **Imagem:** [media - emilia_echo_rivera.jpg](../imagens/media%20-%20emilia_echo_rivera.jpg)
- **Estado:** [Board](../board/board_campanha.md) · [Reputação](../reputacao.md) · [Event Queue](../event_queue.md) · [Registro](../sistema/registro_arquivos.md)
