...
| **F23** | **Leopold von und zu Habsbrück** = Rockerboy da crew (NC, off-screen). Handle = **Prometheus**. Chama ≠ brasão. Slogan = **O FOGO NÃO É DELES.** Tese: *a cidade pertence a quem vive nela*. Líder Zoner **sem nome**. Ele ainda **usa o sobrenome às escondidas** (vergonha; não é trunfo de mesa). Espartana vs caveira = [echo_exposicao](echo_exposicao.md) §7.1. CI **6**. Não narrar no Pack | ficha rockerboy · nota |
| **F24** | **Uncredited** = nome de **saída** (pista, salão, after). Leo + Echo batizaram. ≠ **Void List** (job / vazamento). ≠ “crew do Kaz”. ≠ **Prometheus** (ele). Não usar no feed de op. Crew faz WTF e segue se tiver bebida. Off-screen até NC | crew_relacionamentos · echo_exposicao |
...
| **F16** | **Condor** + **Corujas** = drones miméticos de Ryan, **operacionais** no Pack (demo 20/07). Condor = **unmanned** (gancho/visor; payload = módulo, não pessoas). Envelope e lentes: [drone - condor.md](../fichas/drone%20-%20condor.md). Não confundir com Vespas. Enxame mini-drones = só ideia (21/07) | ficha Condor · loadout · sessão 012 |
...
- Não narrar **Leopold / Prometheus** no Pack. Não despejar teia Habsbrück / avó / job do Kaz / semanas Zoners / grafite na cerca. Handle = **Prometheus**. Slogan = **O FOGO NÃO É DELES.** Tese da causa = *a cidade pertence a quem vive nela*. Líder Zoner **sem nome**. Sobrenome às escondidas = vergonha, não trunfo. Espartana ≠ caveira. ≠ Voodoo Boys (**F23**).
- Não usar **Uncredited** em vazamento / Void List / job. Nome de **saída** só. Não anunciar a crew como Void List numa festa (**F24**).
