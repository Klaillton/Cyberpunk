...
| **F23** | **Leopold von und zu Habsbrück** = Rockerboy da crew (NC, off-screen). Handle = **Prometheus**. Chama ≠ brasão Habsbrück. Slogan = **O FOGO NÃO É DELES.** Espartana = Prometheus no ar **só** se o job for a mensagem dele. Caveira = kit Void List (Leo pinta cada job); Echo odeia e troca a dela; se ele for de caveira, ela **não** diz Prometheus. ≠ Echo. ≠ Voodoo Boys. CI **6**. Não narrar no Pack. “EU SÓ ANUNCIO…” = mais tarde | ficha rockerboy · [echo_exposicao](echo_exposicao.md) §7.1 |
...
- Não narrar **Leopold / Prometheus** no Pack. Não despejar teia Habsbrück / avó / job do Kaz / semanas Zoners / grafite na cerca. Handle = **Prometheus**. Slogan = **O FOGO NÃO É DELES.** Espartana ≠ caveira: espartana só se o job for a mensagem dele; caveira = Echo inventa apelido, nunca Prometheus. ≠ Voodoo Boys (**F23**).
