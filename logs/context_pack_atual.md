# Context Pack atual (tier-0)

**Finalidade:** único arquivo **mínimo** que a IA deve ler/re-ler para não esquecer o estado.  
**Gerado após:** sessão **016** · **Próxima:** **017** (`sessao_resumo_017.md`)  
**Branch:** `feature/linha-estavel`  
**Atualizado:** 23 de Julho de 2026 (tarde)

> **Não é o board.** Detalhe em `board/board_campanha.md`.  
> Fatos estáveis: [sistema/fatos_duros.md](../sistema/fatos_duros.md).  
> Comandos: [sistema/comandos_jogador.md](../sistema/comandos_jogador.md).

---

## NOW

| Campo | Valor |
| ----- | ----- |
| Data in-game | **23/07/2026** |
| Período | Tarde |
| Região | **Badlands** |
| Local específico | Pack Nômade — tenda Ryan & Valk |
| Facção / base local | Pack Badlands (hospedagem) |
| Cena / gancho | Descanso na tenda após preparação do Mule e conversa com Lira/Sasha |
| Prioridade | **Saída 24/07 cedo** — acampamento leve + tentativa contato NC (Lira + Sasha no Mule) |
| Segredos ativos | Casas modulares — time produção + Tio Gringo sabem; pack geral ainda aguarda revelação oficial completa |

### Cena de abertura sugerida

**Manhã 24/07 — Pack / garagem ou tenda.** Saída combinada: Ryan, Valk, Lira e Sasha no Mule. Acampamento leve + tentativa de contato com Sparrow/Steel para localizar Kaz. Todos no Mule; se Sasha quiser voltar, voltam juntos.

---

## Fatos duros em vigor (não negociar)

| ID | Resumo |
| -- | ------ |
| F01–F02 | Branch `feature/linha-estavel`; arquivos = SoT |
| **F03** | Warden = drone **terrestre** scorpion (**não voa**) |
| **F04** | **Stitch** = Stephania (crew); **Doc** = Elisa Moreau (passado) |
| **F14** | **Scout** (pack) ≠ **Jax "Razor" Kane** (crew NC) |
| **F15** | Valk **não** chama Ryan de “herói” / “herói solitário” (promessa 14/07; gatilho ativo) |
| F07 | Ryan × Valk consolidados; decisão conjunta de ir a Night City |
| F08 | Mule = de Valk; equipe Valk + Mule |
| **F11** | Casas modulares — pack geral ainda sem revelação oficial completa |
| **L01** | No Pack: Reyes líder; Tio Gringo forja |
| **L02** | E010 concluída; Container A no pack; Node andando |
| **L03** | Time de produção + Tio Gringo conhecem o projeto |
| **Novo** | Condor + Corujas operacionais (drones miméticos) |
| **Novo** | Modelo 3D externo da base militar coletado (21/07) |
| **Novo** | Lira + Sasha aceitaram acampamento leve 24/07 (todos no Mule) |
| **F18** | Ruleset **v1.3.0** · 017+ · sem retcon 001–016 · [regras_red](../sistema/regras_red/00_integridade_regras.md) · [11_referencia](../sistema/regras_red/11_referencia.md) · [ryan_loadout](../fichas/ryan_loadout.md) |

Lista completa: [fatos_duros.md](../sistema/fatos_duros.md).

---

## Pendências quentes

| ID | Uma linha |
| -- | --------- |
| **E015** | Viagem / assuntos Ryan em Night City (com Valk) — próximo passo: acampamento 24/07 + contato Sparrow/Steel |
| **E019** | Olaria + desidratador + cogeração forja — ideia delegada ao Pack (23/07) |
| **E012** | Interno + móveis casas; possível revelação/escala |
| E007 | Badlands Node |
| E008 | Vigilância residual Raffen |
| E011 | Visita Doc **Moreau** (Elisa); Valk junto |
| E001/E006 | Biotechnica latente |
| Tutoria | Valk + Sasha/Lira (Lira aberta, Sasha reservada; ambas no acampamento) |
| Novo | Enxame mini-drones (ideia + sondagem materiais) |
| Novo | Modelo 3D da base militar (estudo posterior) |

---

## O que acabou de acontecer (016 — 1 parágrafo)

22/07: tutoria Sasha/Lira; planejamento discrição para contato NC (Sparrow/Steel); conversa profunda limites com Valk sobre Sasha/Lira; intimidade. 23/07: preparação Mule; Lira e Sasha aceitam acampamento leve (saída 24/07 cedo, todos no Mule); ideia olaria/desidratador/cogeração apresentada e **delegada ao Pack**; descanso na tenda.

Detalhe: [sessao_resumo_016.md](sessao_resumo_016.md)

---

## Confirmação de boot (formato fixo)

```
Boot OK · 23/07/2026 tarde · Badlands/Pack · prioridade: saída 24/07 acampamento + contato NC · próximo resumo: 017
```

Hierarquia: **RAW/repo > sandbox > memória de chat**.
